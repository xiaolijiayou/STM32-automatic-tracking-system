...
void USART1_IRQHandler(void)
{
	...
	{
		vp_rxbuff[vp_circle_rx_index] = USART1->DR;//将接收到的数据存入缓存数组
		vp_circle_rx_index++;
...
unsigned char readValuePack(RxPack *rx_pack_ptr)
{
   ...
   rdIndex+=rx_pack_length;	
	...
	while(rdIndex<=(rxIndex-rx_pack_length))
	{
		...
		rdii=rdi+1;
		if( vp_rxbuff[rdi]==PACK_HEAD)//包头校验
		{
			if(vp_rxbuff[(rdi+RXPACK_BYTE_SIZE+2)%VALUEPACK_BUFFER_SIZE]==PACK_TAIL)//包尾校验
			{
				//  计算校验和
				sum=0;
